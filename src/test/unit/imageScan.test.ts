...
describe('fail-closed image scanning', () => {
  ...
  it('refunds a live paid lock when image scanning denies execution', async () => {
    const { engine, db } = await makeEngine({ tempFolder, scanImages: false })
    const job = makeJob()
    job.isFree = false
    ...
    db.getJobsByStatus.resolves([job])
    const escrow = {
      getLocks: sinon.stub().resolves([
        {
          jobId: job.jobIdHash,
          expiry: String(Math.floor(Date.now() / 1000) + 300),
          token: job.payment.token,
          payer: job.owner
        }
      ]),
      claimLocks: sinon.stub().resolves('0xrefund'),
      cancelExpiredLocks: sinon.stub().resolves()
    }
    engine.escrow = escrow as any
    engine.keyManager = { getEthAddress: () => job.owner } as any
    ;(engine as any).envs = [{ id: job.environment, fees: { [job.payment.chainId]: [] } }]

    await (engine as any).claimPayments()

    expect(db.getJobsByStatus.firstCall.args[1]).to.include.members([
      C2DStatusNumber.VulnerableImage,
      C2DStatusNumber.ImageScanFailed
    ])
    expect(escrow.claimLocks.calledOnce).to.equal(true)
    expect(escrow.claimLocks.firstCall.args[4]).to.deep.equal([0])
    expect(escrow.cancelExpiredLocks.notCalled).to.equal(true)
    expect(job.payment.cost).to.equal(0)
    expect(job.payment.claimTx).to.equal('0xrefund')
    expect(job.status).to.equal(C2DStatusNumber.JobFinished)
  })

  it('cancels an expired paid lock', async () => {
    const { engine, db } = await makeEngine({ tempFolder, scanImages: false })
    const job = makeJob()
    job.isFree = false
    ...
    const escrow = {
      ...
      claimLocks: sinon.stub().resolves(),
      cancelExpiredLocks: sinon.stub().resolves('0xcancelled')
    }
    engine.escrow = escrow as any
    engine.keyManager = { getEthAddress: () => job.owner } as any
    ;(engine as any).envs = [{ id: job.environment, fees: { [job.payment.chainId]: [] } }]
    ...
    expect(escrow.claimLocks.notCalled).to.equal(true)
    expect(escrow.cancelExpiredLocks.called).to.equal(true)
    expect(escrow.cancelExpiredLocks.firstCall.args).to.deep.equal([
      job.payment.chainId,
      [job.jobId],
      [job.payment.token],
      [job.owner]
    ])
    expect(job.payment.cancelTx).to.equal('0xcancelled')
    expect(job.status).to.equal(C2DStatusNumber.JobFinished)
  })
